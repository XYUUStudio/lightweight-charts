...
export class PaneWidget implements IDestroyable, MouseEventHandlers {
	...
	protected _drawBackground({ context: ctx, bitmapSize }: BitmapCoordinatesRenderingScope): void {
	...
	protected _drawGrid(target: CanvasRenderingTarget2D): void {
	...
	protected _drawWatermark(target: CanvasRenderingTarget2D): void {
	...
	protected _drawSources(target: CanvasRenderingTarget2D, paneViewsGetter: PaneViewsGetter): void {
		const state = ensureNotNull(this._state);
		const sources = state.orderedSources();
	...
	protected _setCrosshairPosition(x: Coordinate, y: Coordinate, event: MouseEventHandlerEventBase): void {
	...
	protected _startTrackingMode(startTrackPoint: Point, crossHairPosition: Point, event: MouseEventHandlerEventBase): void {
